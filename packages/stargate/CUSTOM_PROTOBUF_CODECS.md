...
- You have installed `@uptsmart/proto-signing`, `@uptsmart/stargate` and
  ...
  ```
  "dependencies": {
    "@uptsmart/proto-signing": "^0.26.4",
    "@uptsmart/stargate": "^0.26.4",
...
script we used for the `@uptsmart/stargate` package
...
`@uptsmart/proto-signing` for you to use, which maps type URLs to codec objects.
For example:

```ts
import { DirectSecp256k1HdWallet, Registry } from "@uptsmart/proto-signing";
import {
...
Additionally, we provide a system for extending `@uptsmart/stargate`’s
